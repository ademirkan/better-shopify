...
for link in LINKS:
    ...
    for feature_str in query_params["feature"]:
        ...
        if feature_key.upper() not in ["FABRIC", "POSITION"]:
            ...
            feature_key = feature_key.replace(" ", "%20")
...
for product_key in product_dict:
    ...
    for fabric in FABRICS:
        for position in range(1, MAX_NUM_POSITIONS + 1):
            # Check if position exists
            if not ping(
                f"https://content.cylindo.com/api/v2/4926/products/{product_key}/frames/1/{product_key}.JPG?background={BACKGROUND_COLOR}&feature=FABRIC:565&feature=POSITION:{position}"
            ):
                continue
            ...
            if custom_features != {}:
                ...
                for custom_key, custom_val in custom_features.items():
                    for value in custom_val:
                        ...
                        folder_path = f".innovation-4k-images/{product_key}/{custom_key}_{value}/bg_{BACKGROUND_COLOR}/fabric_{fabric}"
                        for frame in [1, 5]:
                            src = f"https://content.cylindo.com/api/v2/4926/products/{product_key}/frames/{frame}/{product_key}.JPG?background={BACKGROUND_COLOR}&crop=(0,0,4096,3128)&zoom=4k"
                            for feature_key in features:
                                feature_val = features[feature_key]
                                src += f"&feature={feature_key}:{feature_val}"

                            img_name = f"{product_key}_{position}_{frame}.jpeg"

                            fetchAndStore(src, folder_path, img_name)

            else:
                features = {"FABRIC": fabric, "POSITION": position}
                folder_path = f".innovation-4k-images/{product_key}/bg_{BACKGROUND_COLOR}/fabric_{fabric}"
                for frame in [1, 5]:
                    src = f"https://content.cylindo.com/api/v2/4926/products/{product_key}/frames/{frame}/{product_key}.JPG?background={BACKGROUND_COLOR}&crop=(0,0,4096,3128)&zoom=4k"
                    for feature_key in features:
                        feature_val = features[feature_key]
