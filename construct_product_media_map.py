import os
import json
from helpers import file_stage_and_create
...
def increment_progress():
    ...
    print(f"Processed {num_processed_files} of {num_total_files} files", end="\r")
...
def process_file(path, name):
    ...
    if os.path.isdir(path) and name.startswith('360'):
        ...
        if not name.lower().endswith(('.png', '.jpg', '.jpeg', '.mp4')):
            return None
        response = file_stage_and_create(path, name, name)
        if name.lower().endswith(('.png', '.jpg', '.jpeg')):
            return {"type": "IMAGE", "value": response}
        elif name.lower().endswith('.mp4'):
            return {"type": "VIDEO", "value": response}
    return None
...
def process_360_folder(folder_path):
    ...
    for img_file in sorted(os.listdir(folder_path)):
        if img_file.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            images.append(file_stage_and_create(img_path, img_file, img_file))
            increment_progress()
